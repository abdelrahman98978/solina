      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-red-50 text-[#0056B3] text-xs font-bold mb-4 border border-red-100">
            ...
            <span>{language === 'ar' ? 'الثقة والاعتمادية والريادة منذ 1955' : 'Trust, Reliability & Excellence Since 1955'}</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-normal text-black mb-4 font-arabic tracking-tight">
            {language === 'ar' ? 'لماذا تختار سولينا للسيارات مع سولينا للسيارات؟' : 'Why Choose Solina Saudi Arabia with Solina Motors?'}
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {WHY_SOLINA_POINTS.map((point, idx) => (
            ...
            >
              <div>
                <div className="w-14 h-14 rounded-xl bg-white group-hover:bg-red-50 text-[#0056B3] flex items-center justify-center shadow-sm border border-gray-100 mb-6 transition-colors">
                  {getIcon(point.icon)}
                </div>
